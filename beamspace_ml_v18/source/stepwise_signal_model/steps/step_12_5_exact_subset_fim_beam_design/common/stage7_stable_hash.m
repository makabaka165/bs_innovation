function digest = stage7_stable_hash(varargin)
%STAGE7_STABLE_HASH Return a deterministic SHA-256 digest.
...
if istable(value)
    bytes = [uint8('table'), 0, encode_value_local(value.Properties.VariableNames)];
    for index = 1:width(value)
        variable_name = value.Properties.VariableNames{index};
        item = encode_value_local(value.(variable_name));
...
elseif isstruct(value)
    ...
    error('stage7_stable_hash:UnsupportedType', ...
