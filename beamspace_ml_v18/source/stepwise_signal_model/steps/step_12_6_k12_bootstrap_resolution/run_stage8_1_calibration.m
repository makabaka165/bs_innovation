...
[cells, materialization_debug] = materialize_stage8_calibration_cells( ...
    ...
    'cell_indices', opts.cell_indices, 'source_identity', ...
...
shard = run_stage8_1_calibration_shard(cells, plan.local_domain, ...
    registry, stage5_locked, struct('cell_indices', (1:numel(cells)).', ...
    'checkpoint_dir', checkpoint_root, ...
    'cell_options', cell_options));
if numel(opts.cell_indices) == 300
    [complete_artifacts, checkpoint_manifest] = ...
        collect_stage8_1_cell_artifacts( ...
        checkpoint_root, plan.calibration, threshold_contract);
    [thresholds, aggregation_debug] = aggregate_stage8_1_calibration( ...
        complete_artifacts, plan.calibration, struct('formal_run', true, ...
        'expected_contract', threshold_contract));
else
    thresholds = struct([]);
    checkpoint_manifest = table();
...
output = struct('plan', plan, 'measurement_registry', registry, ...
    ...
    materialization_debug, 'shard', shard, 'checkpoint_manifest', ...
    checkpoint_manifest, 'threshold_contract', threshold_contract, ...
    'locked_thresholds', ...
