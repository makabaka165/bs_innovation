...
L = zeros(row_count, 1);
element_seed = zeros(row_count, 1);
auxiliary_seed = zeros(row_count, 1);
...
for stratum_row = 1:height(strata)
    for trial_index = 1:K1.trials_per_stratum
        ...
        for config_index = 1:numel(config_ids)
            ...
            element_seed(row_index) = ...
                strata.element_seed_start(stratum_row) + trial_index - 1;
            auxiliary_seed(row_index) = ...
                strata.auxiliary_seed_start(stratum_row) + trial_index - 1;
        end
    end
end
registry = table(evaluation_row_index, stratum_index, stratum_id, ...
    common_trial_id, pairing_key, trial_index_within_stratum, ...
    measurement_config_id, noise_profile_id, L, element_seed, ...
    auxiliary_seed);
