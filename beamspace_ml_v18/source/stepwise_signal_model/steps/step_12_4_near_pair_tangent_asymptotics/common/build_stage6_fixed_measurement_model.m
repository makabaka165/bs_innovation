...
beam_indices = struct( ...
    'azimuth_local', config.az_beam_indices, ...
    'elevation_local', config.el_beam_indices, ...
    'azimuth_global', config.az_beam_global_indices, ...
    'elevation_global', config.el_beam_global_indices);
fixed_measurement_hash = stable_stage6_hash(config.config_id, ...
    Wseq_hash, Cseq_hash, Tseq_hash, array_geometry_hash, beam_indices, ...
    config.noise_covariance_id, covariance_meta, ...
...
model.stage6_source_tree_hash = opts.stage6_source_tree_hash;
model.stage6_dependency_tree_hash = opts.stage6_dependency_tree_hash;
model.stage6_provenance_hash = opts.stage6_provenance_hash;
model.provenance_contract_version = opts.provenance_contract_version;
...
required = {'stage6_controls_hash','stage6_measurement_plan_hash', ...
    'stage6_experiment_plan_hash','stage6_source_tree_hash', ...
    'stage6_dependency_tree_hash','stage6_provenance_hash', ...
...
if isempty(config.az_beam_deg) || isempty(config.el_beam_deg) || ...
        ...
        numel(config.az_beam_deg) ~= numel(config.az_beam_global_indices) || ...
        numel(config.el_beam_deg) ~= numel(config.el_beam_global_indices)
