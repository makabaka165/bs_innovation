function [manifest, tree_hash] = build_git_blob_manifest( ...
    repo_dir, relative_paths, scope_id, opts)
%BUILD_GIT_BLOB_MANIFEST Resolve tracked index entries and hash identities.

if nargin < 4 || isempty(opts), opts = struct(); end
if ~(isstruct(opts) && isscalar(opts) && isempty(fieldnames(opts)))
    error('build_git_blob_manifest:Options', ...
        'No build_git_blob_manifest options are currently supported.');
end
...
if ~(ischar(scope_id) && isrow(scope_id) && ~isempty(scope_id))
    error('build_git_blob_manifest:Scope', 'scope_id must be nonempty scalar text.');
end
...
for index = 1:numel(paths)
    ...
    if status ~= 0
        error('build_git_blob_manifest:Git', ...
            'git ls-files failed for %s: %s', paths(index), strtrim(output));
    end
    lines = splitlines(string(regexprep(output, '[\r\n]+$', '')));
    lines(lines == "") = [];
    if numel(lines) ~= 1
        error('build_git_blob_manifest:TrackedPath', ...
            'Path must resolve to exactly one stage-0 tracked entry: %s.', paths(index));
    end
    token = regexp(char(lines(1)), ...
        '^([0-9]{6}) ([0-9a-fA-F]{40,64}) ([0-9]+)\t(.+)$', ...
        'tokens', 'once');
    if isempty(token) || ~strcmp(token{3}, '0')
        error('build_git_blob_manifest:IndexEntry', ...
            'Path does not have one unconflicted stage-0 index entry: %s.', paths(index));
    end
    returned_path = strrep(token{4}, '\', '/');
    if ~strcmp(returned_path, char(paths(index)))
        error('build_git_blob_manifest:PathMismatch', ...
...
[manifest, tree_hash] = hash_git_blob_manifest(manifest, scope_id);
end
...
if ~(ischar(repo_dir) && isrow(repo_dir) && exist(repo_dir, 'dir') == 7)
    error('build_git_blob_manifest:RepoDir', ...
...
paths = string(relative_paths(:));
if isempty(paths) || any(ismissing(paths) | strlength(paths) == 0)
    error('build_git_blob_manifest:Paths', 'relative_paths must be nonempty text.');
end
paths = replace(paths, '\', '/');
if numel(unique(paths)) ~= numel(paths)
    error('build_git_blob_manifest:DuplicatePath', ...
        'relative_paths contains a duplicate path.');
end
for index = 1:numel(paths)
    text = char(paths(index));
    if startsWith(text, '/') || ~isempty(regexp(text, '^[A-Za-z]:', 'once')) || ...
            ~isempty(regexp(text, '[\t\r\n"]', 'once')) || ...
            any(strcmp(split(paths(index), '/'), '..'))
        error('build_git_blob_manifest:Path', ...
...
if contains(repo_dir, '"') || contains(arguments, newline)
    error('build_git_blob_manifest:ShellInput', ...
