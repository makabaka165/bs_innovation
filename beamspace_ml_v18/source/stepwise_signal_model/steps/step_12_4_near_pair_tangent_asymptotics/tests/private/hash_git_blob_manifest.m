function [manifest, tree_hash] = hash_git_blob_manifest(manifest, scope_id)
%HASH_GIT_BLOB_MANIFEST Hash sorted Git mode/blob/path records.

if isstring(scope_id), scope_id = char(scope_id); end
if ~(ischar(scope_id) && isrow(scope_id) && ~isempty(scope_id))
    error('hash_git_blob_manifest:Scope', 'scope_id must be nonempty scalar text.');
end
required = {'relative_path','git_mode','git_blob_hash'};
if ~(istable(manifest) && all(ismember(required, manifest.Properties.VariableNames)))
    error('hash_git_blob_manifest:Manifest', ...
        'manifest must contain relative_path, git_mode, and git_blob_hash.');
end

paths = string(manifest.relative_path);
modes = lower(string(manifest.git_mode));
blobs = lower(string(manifest.git_blob_hash));
if any(ismissing(paths) | ismissing(modes) | ismissing(blobs)) || isempty(paths)
    error('hash_git_blob_manifest:MissingValue', ...
        'Manifest identity fields must be nonempty and nonmissing.');
end
paths = replace(paths, '\', '/');
if numel(unique(paths)) ~= numel(paths)
    error('hash_git_blob_manifest:DuplicatePath', ...
        'Manifest paths must be unique.');
end
for index = 1:numel(paths)
    validate_path_local(paths(index));
    if isempty(regexp(char(modes(index)), '^[0-9]{6}$', 'once'))
        error('hash_git_blob_manifest:Mode', 'Invalid Git mode for %s.', paths(index));
    end
    if isempty(regexp(char(blobs(index)), '^[0-9a-f]{40,64}$', 'once'))
        error('hash_git_blob_manifest:Blob', 'Invalid Git blob id for %s.', paths(index));
    end
end
...
if isempty(text) || startsWith(text, '/') || ...
    ...
    error('hash_git_blob_manifest:Path', ...
