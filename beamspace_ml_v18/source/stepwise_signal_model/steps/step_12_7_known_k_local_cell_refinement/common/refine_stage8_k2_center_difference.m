function [estimate, history, debug] = refine_stage8_k2_center_difference( ...
    full_data, initial_angles_deg, local_domain, model, opts)
%REFINE_STAGE8_K2_CENTER_DIFFERENCE Mechanical lift of the Core-V2 solver.

if nargin < 5, opts = struct(); end
contract = k2_contract_local();
validate_options_local(opts, contract);
validate_inputs_local(full_data, initial_angles_deg, local_domain, model);
contract_hash = stage8_core_v2_2_stable_hash( ...
...
if current_valid
    ...
    for sweep = 1:contract.max_sweeps
        ...
        for coordinate = 1:4
            ...
            [global_lower, global_upper] = global_bounds_local(parameters, ...
                coordinate, local_domain.domain_bounds_deg);
        ...
        if last_relative_change <= contract.relative_score_tolerance && ...
                last_endpoint_update <= contract.endpoint_update_tolerance_deg
...
function contract = k2_contract_local()
contract = struct( ...
    'max_sweeps', 20, ...
    'scan_point_count', 9, ...
...
if ~(isstruct(opts) && isscalar(opts))
    error('refine_stage8_k2_center_difference:Options', ...
...
for index = 1:numel(names)
    if ~isequal(opts.(names{index}), contract.(names{index}))
        error('refine_stage8_k2_center_difference:FrozenContract', ...
...
if ~(valid_data && valid_angles && valid_contract)
    error('refine_stage8_k2_center_difference:Input', ...
...
switch coordinate
    ...
    otherwise
        error('refine_stage8_k2_center_difference:Coordinate', ...
