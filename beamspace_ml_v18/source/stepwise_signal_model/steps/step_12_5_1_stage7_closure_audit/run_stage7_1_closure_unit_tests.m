...
%RUN_STAGE7_1_CLOSURE_UNIT_TESTS Run A4 code-only tests without formal results.
...
tests.peak_memory_difference = ...
    test_peak_memory_schema_dependent_difference_passes( ...
    repo_dir, result_dir);
tests.peak_memory_unit = ...
    test_peak_memory_unit_change_fails(repo_dir, result_dir);
tests.peak_memory_status = ...
    test_peak_memory_status_change_fails(repo_dir, result_dir);
tests.peak_memory_pass_flag = ...
    test_peak_memory_pass_flag_change_fails(repo_dir, result_dir);
tests.peak_memory_invalid = ...
    test_peak_memory_invalid_value_fails(repo_dir, result_dir);
tests.non_memory_change = ...
    test_non_memory_keypoint_change_still_fails(repo_dir, result_dir);
tests.deterministic_memory = ...
    test_deterministic_memory_contract(repo_dir, result_dir);
tests.memory_frozen_plan = ...
...
tests.memory_writer_determinism = ...
    test_memory_audit_writer_determinism(repo_dir);
tests.stage7_1_stable_readme = ...
    test_stage7_1_stable_readme_contract(step_dir);
...
fprintf(['Stage7.1A4 code-only tests: %d assertions; analyzer/scope ', ...
...
assert(technical_pass, 'run_stage7_1_closure_unit_tests:Failed', ...
    'At least one Stage7.1A4 code-only gate failed.');
