...
The [first recovery record](58_stage8_k2_raw_tangent_controller_recovery.json) contains the first archive hashes and actual-launch evidence. The real launch test uses the production launch helper and executes a Tick while both launcher and child are present. It requires one compute worker, unchanged state and clean process exit. Current T18 covers six allowed process/timestamp representations and nine rejected inventories, 15 state transitions, and real scheduled-task registration/export/removal. T16 additionally rejects wrong HEAD and wrong source hash. The [fixed-test report](58_stage8_k2_raw_tangent_fixed_tests.json) records the latest complete 18-test rerun.

## PowerShell Timestamp Follow-Up

At recovery commit `7c4b95a750875c53d554056bc42b01c54b49408e`, a manual PowerShell 7 Tick exposed another identity parsing error. PowerShell 7 decodes ISO JSON timestamps as DateTime values, while Windows PowerShell 5.1 leaves strings. Passing an already typed value through DateTime.Parse first converted it to a culture-formatted string, losing the UTC kind and fractional seconds. The single valid worker was falsely rejected. Both task-owned processes were stopped and all 61 complete checkpoints were verified, including observation reconstruction and 183 metric rows, then preserved at `backup/timestamp_incident_7c4b95a`.

The corrected conversion accepts both strings and typed DateTime values without losing UTC or fractional seconds. The real MATLAB fixture now invokes the same Tick in both PowerShell 5.1 and 7 and fails if the mutex cannot be acquired. Both host editions must pass before T18 accepts the evidence. The [timestamp recovery record](58_stage8_k2_raw_tangent_timestamp_recovery.json) records the second archive and current validation. The 61 trials are recomputed under the final recovery revision with the same exact checkpoint identity checks. The first archive remains untouched.
