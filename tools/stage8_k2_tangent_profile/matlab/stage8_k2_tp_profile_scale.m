...
result.runtime_sec = toc(clock);
    ...
    function evaluation = evaluate_rho_local(rho)
        ...
        if manifold_info.rank_Gseq < 2
            ...
            audit_record_query_local(evaluation.angles_deg, G, ...
                manifold_info, -Inf, false, model, ...
        ...
        audit_record_query_local(evaluation.angles_deg, G, ...
            manifold_info, score, true, model, constants.rank_multiplier);
...
function audit_record_query_local(angles, G, info, score, scored, model, ...
    rank_multiplier)
if exist('stage8_k2_tcc_audit_state', 'file') ~= 2 || ...
        ~stage8_k2_tcc_audit_state('QUERY_ENABLED')
    return;
end
event = struct('stage_id', 'T4_PROFILE', 'angles_deg', angles, ...
    'G', G, 'direct_rank', info.rank_Gseq, 'direct_score', score, ...
