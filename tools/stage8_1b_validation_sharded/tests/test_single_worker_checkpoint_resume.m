...
stage8_1b_write_checkpoint_audit(path_now, protocol, first);
before_hash = stage8_sha256_file(path_now);
second = stage8_1b_validate_checkpoint(path_now, protocol, expected_rows);
audit_record = jsondecode(fileread([path_now, '.audit.json']));
...
assert(strcmp(string(audit_record.checkpoint_file_sha256), string(after_hash)));
