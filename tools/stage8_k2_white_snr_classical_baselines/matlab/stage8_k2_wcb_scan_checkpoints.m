function scan = stage8_k2_wcb_scan_checkpoints( ...
    runtime_root, registry, context, registry_hash)
%STAGE8_K2_WCB_SCAN_CHECKPOINTS Validate every existing final checkpoint.

checkpoint_dir = fullfile(runtime_root, 'checkpoints');
if ~isfolder(checkpoint_dir)
    error('stage8_k2_wcb_scan_checkpoints:Directory', ...
        'Checkpoint directory is missing.');
end
expected_names = strings(height(registry), 1);
for index = 1:height(registry)
    [~, name, extension] = fileparts(stage8_k2_wcb_checkpoint_path( ...
...
if ~isempty(unexpected)
    error('stage8_k2_wcb_scan_checkpoints:Unexpected', ...
...
for index = 1:height(registry)
    path_now = stage8_k2_wcb_checkpoint_path( ...
        runtime_root, registry(index, :));
    if isfile(path_now)
        checkpoints{index} = stage8_k2_wcb_checkpoint_load( ...
            path_now, registry(index, :), context, registry_hash);
        completed(index) = true;
        runtime_sec(index) = checkpoints{index}.runtime_sec;
        if isfile([path_now, '.tmp'])
            error('stage8_k2_wcb_scan_checkpoints:FinalAndTemporary', ...
...
if ~isempty(unexpected_tmp)
    error('stage8_k2_wcb_scan_checkpoints:UnexpectedTemporary', ...
