function report = stage8_k2_rtc_audit_incident(repo,runtime)
cleanup=stage8_k2_rtc_add_paths(repo); %#ok<NASGU>
formal=jsondecode(fileread(fullfile(runtime,'controller','formal_identity.json')));
assert(strcmp(formal.head,'d4e2517bf860efeabbf40925af44ba17bff85495'));
assert(strcmp(formal.source_hash,'a0907109d220e20ea993fd225ad9513fde2a93f097e33b914014846ac05d75ff'));
...
assert(numel(files)==45 && isempty(dir(fullfile(runtime,'checkpoints','element','*.mat'))));
...
stage8_k2_rtc_write_json(fullfile(runtime,'controller','incident_readonly_audit.json'),report);
fprintf('INCIDENT AUDIT PASS: 45 checkpoints, 135 metric rows, all old source hashes verified.\n');
end
